...
% 演題情報
% -----------------------------------------------------------------------------
\title[大数の弱法則とBernstein多項式]{大数の弱法則とその応用\\ --- Bernstein多項式による近似定理 ---}
...
\begin{frame}{1. 導入：確率収束とは何か}
    ...
    \begin{definition}[確率収束 (Convergence in Probability)]
        確率変数列 $\{X_n\}$ が確率変数 $X$ に\textbf{確率収束}するとは、任意の $\varepsilon > 0$ に対して以下が成り立つことをいう：
        \[
...
\begin{frame}{2. 証明の道具：チェビシェフの不等式}
    \begin{theorem}[チェビシェフの不等式 (Chebyshev's Inequality)]
        $\phi: \mathbb{R} \to \mathbb{R}$ を非負関数とし、$A \in \mathcal{B}(\mathbb{R})$ に対して $i_A = \inf \{\phi(x) : x \in A\}$ とおく。
...
% -----------------------------------------------------------------------------
\section{応用例（Bernstein多項式による近似定理）}

\begin{frame}{3. 応用：ワイエルシュトラスの近似定理へのアプローチ}
    \begin{theorem}[ワイエルシュトラスの近似定理]
        閉区間 $[0, 1]$ 上の連続関数 $f(x)$ に対し、ある多項式関数列 $f_n(x)$ が存在して $\lim_{n \to \infty} \sup_{x \in [0, 1]} |f_n(x) - f(x)| = 0$
    \end{theorem}

    \pause
    \begin{exampleblock}{Bernstein 多項式と確率論的アイデア}
    ...
    \pause
    \textbf{ポイント}: 標本平均 $\frac{S_n}{n}$ は大数の法則より $x$ に確率収束する。\\
    \textbf{結論}: 大数の法則を用いることで、$f_n(x) \to f(x)$ の一様収束が直接導ける。（※証明の詳細はレポート参照）
\end{frame}
...
\begin{frame}{4. まとめと発展}
    \begin{block}{まとめ}
        \begin{itemize}
            \item 分散が有限な場合の大数の弱法則は、チェビシェフの不等式により容易に導出できる。
            \item 二項分布と組み合わせることで、Weierstrassの近似定理 を確率論的に証明できる。
        \end{itemize}
    \end{block}

    \pause
    \begin{alertblock}{レポートの後半では条件を「期待値の存在 $E[|X|] < \infty$」のみへ}
        分散が無限大になり得るため、分散を用いた証明が破綻する。
        \begin{itemize}
            \item 確率変数の切断 (Truncation)：有界化（$|\bar{X}_{n, k}| \le n$）により、標本平均$\frac{\bar{S_n}}{n}$の分散を有限化
              \[
                  \bar{X}_{n, k} = X_k 1_{\{|X_k| \le n\}}
              \]

            \item ルベーグの優収束定理による尾部評価：$|X_k| 1_{\{|X_k| \le n\}} \le |X_1| \in L^1$ より極限順序交換が可能
              \[
                  \lim_{x \to \infty} x P(|X_1| > x) \le \lim_{x \to \infty} E\left( |X_1| 1_{\{|X_1| > x\}} \right) = E\left( \lim_{x \to \infty} f_x \right) = 0
              \]
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Appendix：Weierstrassの近似定理への確率論的アプローチ}
...
\begin{frame}{証明のアイデア：確率変数の導入}
    ...
    \pause
    したがって、$f\left(\frac{S_n}{n}\right)$ の期待値を計算すると、Bernstein多項式そのものになる！
    \[
